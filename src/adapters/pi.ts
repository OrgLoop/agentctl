...
export class PiAdapter implements AgentAdapter {
  ...
  private extractSessionIdFromFilename(filename: string): string | undefined {
  ...
  private async isSessionRunning(
    ...
    const meta = await readSessionMeta(this.sessionsMetaDir, disc.sessionId);
    if (meta?.pid) {
      ...
      if (this.isProcessAlive(meta.pid)) {
        ...
        if (pidInfo?.startTime && meta.startTime) {
          ...
          if (
            ...
            await deleteSessionMeta(this.sessionsMetaDir, disc.sessionId);
            return false;
          }
        }
  ...
  private async getFirstPrompt(filePath: string): Promise<string | undefined> {
    try {
      const content = await fs.readFile(filePath, "utf-8");
      for (const line of content.split("\n").slice(0, 20)) {
        try {
  ...
  private async findSession(
    ...
    return (
      all.find(
        (d) => d.sessionId === sessionId || d.sessionId.startsWith(sessionId),
      ) || null
    );
  }
...
async function getPiPids(): Promise<Map<number, PidInfo>> {
  ...
  try {
    ...
    for (const line of stdout.split("\n")) {
      ...
      if (!command.startsWith("pi -") && !command.startsWith("pi --")) continue;
