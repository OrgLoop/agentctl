...
describe("PiAdapter", () => {
	...
	describe("list()", () => {
		...
		it("sorts running sessions first, then by most recent", async () => {
			const olderCreated = new Date("2026-02-15T10:00:00Z");
			const newerCreated = new Date("2026-02-17T10:00:00Z");
	...
	describe("peek()", () => {
		it("returns recent assistant messages", async () => {
			...
			const output = await adapter.peek(
				"peek-session-0000-000000000000",
			);
		...
		it("respects line limit", async () => {
			...
			const output = await adapter.peek(
				"limit-session-0000-000000000000",
				{ lines: 3 },
			);
	...
	describe("status()", () => {
		it("returns session details", async () => {
			...
			const session = await adapter.status(
				"status-session-0000-000000000000",
			);
		...
		it("returns first user prompt", async () => {
			...
			const session = await adapter.status(
				"prompt-session-0000-000000000000",
			);
	...
	describe("token aggregation", () => {
		it("sums tokens across multiple assistant messages", async () => {
			...
			const session = await adapter.status(
				"token-session-0000-000000000000",
			);
	...
	describe("session lifecycle — detached processes", () => {
		it("session shows running when persisted metadata has live PID", async () => {
			...
			await fs.writeFile(
				path.join(
					sessionsMetaDir,
					"detached-session-0000-000000000000.json",
				),
				JSON.stringify(meta),
			);
		...
		it("session shows stopped when persisted PID is dead", async () => {
			...
			await fs.writeFile(
				path.join(
					sessionsMetaDir,
					"dead-detached-0000-0000-000000000000.json",
				),
				JSON.stringify(meta),
			);
		...
		it("detects PID recycling in persisted metadata via start time", async () => {
			...
			await fs.writeFile(
				path.join(
					sessionsMetaDir,
					"meta-recycle-0000-0000-000000000000.json",
				),
				JSON.stringify(meta),
			);
		...
		it("old metadata without startTime but with live PID assumes running", async () => {
			...
			await fs.writeFile(
				path.join(
					sessionsMetaDir,
					"meta-notime-0000-0000-000000000000.json",
				),
				JSON.stringify(meta),
			);
	...
	describe("session lifecycle scenarios", () => {
		it("wrapper dies → Pi continues → status shows running", async () => {
			...
			await fs.writeFile(
				path.join(
					sessionsMetaDir,
					"wrapper-dies-0000-0000-000000000000.json",
				),
				JSON.stringify(meta),
			);
		...
		it("Pi completes → status shows stopped", async () => {
			...
			await fs.writeFile(
				path.join(
					sessionsMetaDir,
					"pi-complete-0000-0000-000000000000.json",
				),
				JSON.stringify(meta),
			);
		...
		it("old PID recycled → old session shows stopped, not running", async () => {
			...
			await fs.writeFile(
				path.join(
					sessionsMetaDir,
					"recycled-victim-0000-000000000000.json",
				),
				JSON.stringify(meta),
			);
		...
		it("session ID is not pending- when metadata has real ID", async () => {
			...
			expect(sessions).toHaveLength(1);
			expect(sessions[0].id).toBe(
				"real-uuid-abcd-1234-5678-000000000000",
			);
	...
	describe("edge cases", () => {
		it("handles empty JSONL file gracefully", async () => {
			...
			await fs.mkdir(slugDir, { recursive: true });
			await fs.writeFile(
				path.join(slugDir, "1000_empty-session.jsonl"),
				"",
			);
